sap.ui.define([
	...
	return BaseController.extend("opensap.orders.controller.Detail", {
		...
		onOrderChange: function(oEvent){
			...
			// get the relative drop position
			var sDropPosition = oEvent.getParameter("dropPosition");
			// get the list for reordering
			var oList = this.byId("lineItemsList");
			// get the index of dragged item
			var iDraggedIndex = oList.indexOfItem(oDraggedItem);
			// get the index of dropped item
			var iDroppedIndex = oList.indexOfItem(oDroppedItem);
			// get the new index for the dragged item
			var iNewDroppedIndex = iDroppedIndex + (sDropPosition === "Before" ? 0 : 1) + (iDraggedIndex < iDroppedIndex ? -1 : 0);
			// delete the dragged item from the list
			oList.removeItem(oDraggedItem);
			// re-insert the dragged item into the list with new position
		...
		onDelete: function (oEvent) {
			// delete the draged item
			var oItemToDelete = oEvent.getParameter("draggedControl");
			// delete selected item or first item if draged item is null
			if (!oItemToDelete) {
				var oList = this.byId("lineItemsList");
				oItemToDelete = oList.getSelectedItem() || oList.getItems()[0];
			}

			//delete the item after user confiramtion
